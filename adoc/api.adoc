= Common API
TBT LLC <info@tbt-post.net>
1.4.1, 2020-06-01: Public API
:toc: right
:toclevels: 4
{empty}
...
Доступ к API осуществляется исключительно по протоколу HTTP версии не ниже 1.0 с использованием TLS (HTTPS)

ERP апи -- https://api.tbt-post.net

Апи для приложений:

- https://api-next.tbt-post.net
- https://api-staging.tbt-post.net

Версия апи для разработки -- https://sandbox-api.tbt-post.net
...
Альтернативная нотация (новые методы):

{API}/{context}/{method}/{param:<name>}?{get_params}

[horizontal]
    {API}:: API URL base
    {context}:: контекст выполнения
    {method}:: метод
    {param:<name>}:: параметр метода и его имя (если есть, может быть 1 и более)
...
Формат обмена данными:
     ...
     * REQUEST BODY - для передачи данных
...
NOTE: Система использует форматы `UUID` версий `v0`,`v1`,`v2`,`v4` и `v5` (формат `v3` поддерживается, но не используется)
...
[source, shell]
----
curl -v GET "https://sandbox-api.tbt-post.net/api/v1/op/quasi-parcel/394c8e1f-e196-4d3f-9c1d-c021c9837f23" \
-H "Content-Type: application/json" \
-b cookie-jar1.txt
----
...
==== Получение общих данных стоимости услуг

[horizontal]
protocol_method:: GET
method_name:: service-prices
method_params:: ----
get_params:: -----
request_body:: ----
expected_result:: 200 {
  ...
  "office_delivery": {
    ...
    "return_delivery_price_ratio": 0.8,
    "intercity_delivery_prices": [
      {
        "price": <PRICE>,
        "weight": 500
      },
...
| warehousing
| Храниние посылки на отделении. День - Цена.


| cash_payments
| Комиссия за перевод средств. Например: 1 + cod_amount × 0.012


| office_delivery (local_delivery_prices)
| Цены на доставку в пределах города. Цена - Вес.


| office_delivery (intercity_delivery_prices)
| Цены на доставку между городами. Цена - Вес.


| return_delivery_price_ratio
| Соотношение цены обратной доставки к доставке.


| delivery_limits
| Лимиты на доставку. Вес в граммах, размеры и объем в сантиметрах


| explicit_redirect
| Цена на переадресацию груза.

| delivery_insurance
| Страхование, от заявленной стоимости.
...
[source, shell]
----
curl -v -X GET "https://sandbox-api.tbt-post.net/api/v1/service-prices"
----

Ответ::
HTTP/1.1 200 OK +
Server: nginx/1.10.3 +
Date: Tue, 12 Nov 2019 10:31:31 GMT +
Content-Type: application/json; charset=utf-8 +
Content-Length: 865 +
Connection: keep-alive +
Etag: "60d8ea3549e0af7d7a960ded2680290353489a49" +
...
[horizontal]
protocol_method:: POST
method_name:: parcel-delivery-cost
method_params:: ----
get_params:: -----
request_body:: {"cod_amount":<PRICE>,"declared_price":<PRICE>,"receiver_office_id":"<OFFICE_ID>","sender_office_id":"OFFICE_ID","weight":<WEIGHT>,"width":<WIDTH>,"height":<HEIGHT>,"depth":<DEPTH>}
expected_result:: 200 {"cod_payments_cost":<PRICE>,"delivery_cost":<PRICE>,"success":true,"return_delivery_cost":<PRICE>}

NOTE: cod_amount - опционально, если COD установлен. Вес в граммах.
...
[source, shell]
----
curl -v -X POST "https://sandbox-api.tbt-post.net/api/v1/parcel-delivery-cost" \
-H "Content-Type: application/json" \
-d '{"cod_amount":2510,"declared_price":3222,"receiver_office_id":"93ccfdec-f886-4f1c-9d04-6d1d8ef502f3","sender_office_id":"26b10dbb-4f1b-4d45-9c19-ec3cb85d4253","weight":2000,"width":20,"height":20,"depth":50}'
----

Ответ::
HTTP/1.1 200 OK +
Server: nginx/1.10.3 +
Date: Tue, 12 Nov 2019 11:21:09 GMT +
Content-Type: application/json; charset=utf-8 +
Content-Length: 100 +
Connection: keep-alive +
...
----
{
  "cod_payments_cost": 31.12,
  "delivery_cost": 66.11,
  "success": true,
  "return_delivery_cost": 56.11
}
----
